...
program
  ...
  .option("-o, --output <file>", "path of the output file (default: './enums.ts')", "./enums.ts")
  .parse(process.argv);
...
const replacers: Replacer[] = [{ regExp: /[-/]/g, replaceWith: "_" }, { regExp: /[.]/g, replaceWith: "__" }];
...
const parseValue = (value: string) => {
  ...
  if (options.parse) {
    replacers.forEach(({ regExp, replaceWith }) => { inputValue = inputValue.replaceAll(regExp, replaceWith);});
  }
...
const arrayToEnum = (enums: string[], enumName: string) => {
  const enumString = enums.map((value) => { 
...
const getYamlSchemas = (file: string) => {
  const parsedYaml = yaml.parse(file) as OpenAPIV3.Document;
...
const collectEnums = (schemas: OpenAPIV3.ComponentsObject["schemas"]) => {
  ...
  for (const key in schemas) {
    if (typeof schemas[key] === "object" && !("$ref" in  schemas[key]) && "enum" in schemas[key] && !options.exclude?.includes(key)) {
